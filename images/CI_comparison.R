...
set.seed(123)
ala_mat1 <- results_gibbs_map$lower_CI[ lower.tri(results_gibbs_map$lower_CI) ]  # Matriisi 1 alarajat
yla_mat1 <- results_gibbs_map$upper_CI[ lower.tri(results_gibbs_map$lower_CI) ]  # Matriisi 1 ylärajat
ala_mat2 <- results_GEM_map$lower_CI[ lower.tri(results_gibbs_map$lower_CI) ]  # Matriisi 2 alarajat
yla_mat2 <- results_GEM_map$upper_CI[ lower.tri(results_gibbs_map$lower_CI) ]  # Matriisi 2 ylärajat
map1 <- results_gibbs_map$median[ lower.tri(results_gibbs_map$lower_CI) ]  # MAP pisteet matriisi 1
map2 <- results_GEM_map$MAP_estimate[ lower.tri(results_gibbs_map$lower_CI) ]  # MAP pisteet matriisi 2
...
# Järjestä aineisto MAP-pisteiden mukaan
order_map <- order(map1)  # Järjestys map1:n mukaan
...
par(mfrow=c(1,1))
setEPS()
cairo_ps("CI_GEM_GIBBS_CI.eps", width = 20, height = 10, fallback_resolution = 1000)   # koko oletuksena tuumina


# Piirretään credible intervalit
